 ...
 */
class IntersectionsModel extends ViewModel {
	...
	$sql = "SELECT * FROM TrafficJunction";
	try {
	    $this->setContent($this->db->fetchAll($sql), "intersections");
    ...
    public function getIntersectionInfo($int_id) {
	$sql = "SELECT * FROM TrafficJunction WHERE junction_id = :id";
	try {
	    $this->setContent($this->db->fetch($sql, array(":id" => $int_id)), "details");
	} catch (Exception $e) {
